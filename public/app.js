...
async function playMovie(id, name, icon) {
  const creds = getLogin() || {};
  // URL do servidor IPTV
  const host     = creds.host || "";
  const username = creds.username || "";
  const password = creds.password || "";
  const streamUrl = `${host}/movie/${username}/${password}/${id}.m3u8`;
...
function playEpisode(id, title) {
  ...
  const username = creds.username || "";
  const password = creds.password || "";
  const streamUrl = `${host}/series/${username}/${password}/${id}.m3u8`;
...
function loadStream(url, type, id, useProxy = false) {
  ...
  const src = useProxy ? `/api/proxy?url=${encodeURIComponent(url)}&token=${getToken()}` : url;
